...
async function getAPODData(): Promise<{ apod: APODData | null; error?: string }> {
  ...
  if (!NASA_API_KEY) {
    ...
    return {
      error:
        'NASA API Key is missing. Please add NEXT_PUBLIC_NASA_API_KEY to your .env.local file.',
      apod: null,
    };
  }
  ...
  try {
    ...
    if (!response.ok) {
      ...
      throw new Error(
        `API Error: ${response.status} ${response.statusText} - ${
          errorData.msg || 'Unknown error from NASA API.'
        }`
      );
    }
    ...
    const errorMessage =
      error instanceof Error
        ? error.message
        : 'An unexpected error occurred while fetching Astronomy Picture of the Day.';
    console.error('Failed to fetch APOD:', error);
    return { error: errorMessage, apod: null };
  }
}
...
export default async function DashboardPage() {
  ...
  return (
    <>
      <h1 className="text-4xl md:text-5xl font-extrabold mb-6 text-center text-blue-300 relative z-20">
        Welcome to the NASA Data Hub!
      </h1>
      <p className="text-lg md:text-xl text-center mb-10 text-gray-300 relative z-20">
        Your portal to explore the wonders of the universe.
      </p>
      ...
      {error ? (
        <div className="bg-red-700/80 p-4 rounded-lg text-white text-center max-w-3xl mx-auto shadow-lg border border-white relative z-20">
          ...
          <p className="mt-4 text-sm">
            Please ensure your NASA API key is correctly set in{' '}
            <code className="font-mono bg-red-800 px-1 py-0.5 rounded">
              .env.local
            </code>{' '}
        ...
        <div className="p-6 md:p-8 rounded-lg shadow-xl max-w-4xl mx-auto border border-white relative z-20">
          <h2 className="text-2xl md:text-3xl font-bold mb-3 text-center text-blue-200">
            {apod.title}
          </h2>
          ...
          {apod.media_type === 'image' ? (
            <Image
              ...
              width={1200}
              height={630}
              unoptimized // For external images not added to next.config.js domains
          ...
          <p className="text-gray-100 leading-relaxed text-base md:text-lg bg-gray-800/50 p-3 rounded">
            {apod.explanation}
          </p>
        </div>
      ) : (
      ...
      <div className="mt-16 text-center relative z-20">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          <div className="bg-gray-800/70 p-6 rounded-lg shadow-lg hover:bg-gray-700/80 transition duration-300 border border-white">
            ...
            <Link
              href="/exoplanets"
              className="text-blue-400 hover:text-blue-300 hover:underline mt-4 inline-block font-medium"
            >
          ...
          <div className="bg-gray-800/70 p-6 rounded-lg shadow-lg hover:bg-gray-700/80 transition duration-300 border border-white">
            ...
            <p className="text-gray-300">
              See the Red Planet through rover eyes.
            </p>
            <Link
              href="/mars-rover"
              className="text-blue-400 hover:text-blue-300 hover:underline mt-4 inline-block font-medium"
          ...
          <div className="bg-gray-800/70 p-6 rounded-lg shadow-lg hover:bg-gray-700/80 transition duration-300 border border-white">
            ...
            <p className="text-gray-300">
              Track upcoming launches and the International Space Station.
            </p>
            <Link
              href="/launch-tracker"
              className="text-blue-400 hover:text-blue-300 hover:underline mt-4 inline-block font-medium"
